              ...
              <v-menu open-on-hover offset-y>
                <template v-slot:activator="{ on, attrs }">
                  ...
                  <v-btn dark v-bind="attrs" v-on="on" text>
                    ...
                    <span class="teal--text mr-3 text-capitalize font-weight-bold text-caption mb-n2">
                        {{ user.full_name }}
                    </span>
                  </v-btn>
          ...
          <v-card-text>
            ...
            <v-row class="mt-n2" >
                <p class="text-caption pa-4 font-weight-normal">
                    {{ show_first_fifty(user.work_profile.about )}}
                    <span id="dots-1" :style="user.work_profile.show_more ? 'display: none;' : 'display: inline;'">....</span> <span id="more-1" :style="user.work_profile.show_more ? 'display: inline;' : 'display: none;'">
                        {{ show_hidden_description(user.work_profile.about )}}
                    </span>
                    <!-- Button for showing more description -->
                    <v-btn text @click="showMore" x-small icon class="ml-2">
                      <v-icon x-small color="info">
                        {{user.work_profile.show_more ? 'mdi-eye-off' : 'mdi-eye'}}
                      </v-icon>
                    </v-btn>
                    <!-- End of button for wshoing the description -->
                    <br>
                    <v-btn text @click="showDescriptionDialog" x-small class="ml-n1 mt-2" color="info">
                      <v-icon small color="info" left>
                        mdi-account-edit
                      </v-icon>
                      <span class="text-caption font-weight-bold text-capitalize">edit</span>
                    </v-btn>
                </p>
          ...
          <v-card-actions class="mt-n9">
            ...
            <div class="ml-3 mb-1 mt-n3">                
              <span class="text-caption font-weight-normal teal--text ml-2">
                  {{ user.work_profile.job_success}}% Job Success
              </span>
              <v-progress-linear 
                dark 
                :value="user.work_profile.job_success" 
                class="mt-1" 
                rounded height="8" 
                color="cyan" 
                style="width: 180px;">
                ...
                <v-row class="mt-n1 mb-n2">
                    ...
                    <v-col class="">
                        <v-rating
                            ...
                            dense
                            small
                            :value="user.work_profile.rating"
                            class="ml-n10"
                            readonly
                        ></v-rating>
                    </v-col>
        ...
        async uploadSuccess(file, response) {
            ...
            await this.$auth.fetchUser()
                .then(() => {
                    ...
                    this.$store.commit('user_profile/UPDATE_PICTURE', {image: this.$auth.user.profile_pic})
                    // set the message
